...
template <class T>
Nodo<T>::Nodo(Nodo<T>* next, T* task) : siguienteOrdinario(next), Casilla(task){}

template <class T>
    void Nodo<T>::setSO(Nodo* SO) {
...
template<class T>
NodoE<T>::NodoE(Nodo<T>* next, T* task, NodoE<T>* SE) : Nodo<T>(next, task), siguienteExtra(SE) {}

template<class T>
void NodoE<T>::setSE(NodoE<T>* NSE) {
		siguienteExtra = NSE;
	}

template<class T>
    Nodo<T>* NodoE<T>::getSE() {
...
void construirTablero() {
	...
	// Rojo
	auto R12 = new Nodo<Casilla>(V1, new CasillaRecorrido(0, 0, 'N'));
    ...
    // Azul
    auto Z12 = new Nodo<Casilla>(R1, new CasillaRecorrido(0, 0, 'N'));
